                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="{{ route('add_schedule') }}" method="post" id="add-form">
                                    @csrf
                                    <x-librarian.input.basic label="Tanggal mulai" name="tanggal_mulai"
                                        :value="old('tanggal_mulai')" placeholder="" type="date" :isrequired="true" />
                                    <x-librarian.input.basic label="Tanggal selesai" name="tanggal_selesai"
                                        :value="old('tanggal_selesai')" placeholder="" type="date" :isrequired="false" />
                                    <x-librarian.input.basic label="Keterangan" name="keterangan" :value="old('keterangan')"
                                        placeholder="Masukkan keterangan" type="text" :isrequired="true" />
                                    <x-librarian.input.pure-select label="Pilih warna" name="warna" :options="['Hijau', 'Kuning', 'Merah']"
                                        :isrequired="true" />
                ...
                <tbody>
                    @foreach ($schedules as $item)
                        <tr>
                            ...
                            @endif
                            <td align="center">
                                <x-librarian.button.delete :id="$item->id" :route="route('delete_schedule', $item->id)" />
                            </td>
                        </tr>
                    @endforeach
                </tbody>
