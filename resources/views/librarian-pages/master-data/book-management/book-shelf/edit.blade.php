            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('update_shelf', $data->id) }}" method="POST" id="update-form">
                        @method('PUT')
                        @csrf
                        <x-librarian.input.basic label="Kode rak" name="kode" :value="old('kode', $data->kode)"
                            placeholder="Masukkan kode rak" type="text" :isrequired="true" />
                        <x-librarian.input.basic label="Nama rak" name="nama_rak" :value="old('nama_rak', $data->nama_rak)"
                            placeholder="Masukkan nama rak" type="text" :isrequired="true" />
                        <x-librarian.input.basic label="Kapasitas rak" name="kapasitas" :value="old('kapasitas', $data->kapasitas)"
