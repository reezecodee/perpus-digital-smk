        ...
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="row">
                        <div class="col-md-6">
                            <x-librarian.input.basic label="Username" name="username" :value="old('username', $data->username ?? '')"
                                placeholder="Masukkan username" type="text" :isrequired="true" />
                        </div>
                        <div class="col-md-6">
                            <x-librarian.input.basic label="Nama lengkap" name="nama" :value="old('nama', $data->nama ?? '')"
                                placeholder="Masukkan nama lengkap" type="text" :isrequired="true" />
                        </div>
                        <div class="col-md-6">
                            <x-librarian.input.basic :label="'Nomor Induk' . in_array($role, ['Admin', 'Pustakawan'])
                                ? 'Pegawai (NIP)'
                        ...
                        <div class="col-md-6">
                            <x-librarian.input.basic label="Nomor telepon" name="telepon" :value="old('telepon', $data->telepon ?? '')"
                        ...
                        @if ($role == 'peminjam')
                            <div class="col-md-6">
                                <x-librarian.input.basic label="NISN" name="nisn" :value="old('nisn', $data->nisn ?? '')"
                                    placeholder="Masukkan nomor nisn" type="number" :isrequired="true" />
                            </div>
                        @endif
                        <div class="col-md-6">
                            <x-librarian.input.select label="Jenis kelamin" name="jk" :options="['Laki-laki', 'Perempuan']"
                                :selectedvalue="$data->jk ?? ''" :isrequired="true" placeholder="Pilih jenis kelamin" />
                        </div>
                        @if (!isset($data->status) || $data->status == 'Nonaktif')
                            <div class="col-md-6">
                                <x-librarian.input.basic label="Password sementara" name="password_temporary" :value="old('password_temporary')"
                                    placeholder="Masukkan password sementara" type="password" :isrequired="$data ? false : true" />
                            </div>
                            <div class="col-md-6">
                                <x-librarian.input.basic label="Konfirmasi password" name="confirm_password"
                        ...
                        <div class="col-md-12">
                            <x-librarian.input.select label="Status akun" name="status" :options="['Aktif', 'Nonaktif']" :selectedvalue="$data->status ?? ''" :isrequired="true" placeholder="Pilih status akun" />
                        </div>
                        @if ($role == 'admin')
