...
class HelpController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        if ($request->ajax()) {
            $helps = Help::query()
                ->select(['id', 'pelapor_id', 'kategori', 'subject', 'laporan', 'created_at'])
                ->latest();

            return DataTables::of($helps)
                ->addIndexColumn()
                ->addColumn('pelapor', function ($help) {
                    return $help->user->nama;
                })
                ...
                ->editColumn('created_at', function ($help) {
                    return $help->created_at->translatedFormat('d F Y H:i:s');
                })
                ->addColumn('action', function ($help) {
                    $detailUrl = route('detail_help', $help->id);
