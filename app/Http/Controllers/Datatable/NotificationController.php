    ...
    public function __invoke(Request $request)
    {
        if ($request->ajax()) {
            $notifications = Notification::query()
                ->select(['id', 'pengirim_id', 'penerima_id', 'judul', 'pesan', 'status', 'tgl_pengiriman'])->with('receiver')->where('pengirim_id', auth()->user()->id)
                ->latest();

            return DataTables::of($notifications)
                ->addIndexColumn()
                ->addColumn('nama', function ($notification) {
                    return $notification->receiver->nama;
                })
